...
//	char Data_L;
	...
	CLCD_Delay(1);
	Data_I2C[3] = Data_L;
	HAL_I2C_Master_Transmit(LCD->I2C, LCD->ADDRESS, (uint8_t *)Data_I2C, sizeof(Data_I2C), (uint32_t)1000);
}
...
void CLCD_I2C_SetCursor(CLCD_I2C_Name* LCD, uint8_t Xpos, uint8_t Ypos)
{
	...
	if(Xpos >= LCD->COLUMS)
	{
		Xpos = (uint8_t)(LCD->COLUMS - (uint8_t)1);
	}
	if(Ypos >= LCD->ROWS)
	{
		Ypos = (uint8_t)(LCD->ROWS - (uint8_t)1);
	}
	if(Ypos == (uint8_t)0)
	{
		DRAM_ADDRESS = (uint8_t)0x00 + Xpos;
	}
	else if(Ypos == (uint8_t)1)
	{
		DRAM_ADDRESS = (uint8_t)((uint8_t)0x40 + Xpos);
	}
	else if(Ypos == (uint8_t)2)
	{
		DRAM_ADDRESS = (uint8_t)((uint8_t)0x14 + Xpos);
	}
	else if(Ypos == (uint8_t)3)
	{
		DRAM_ADDRESS = (uint8_t)((uint8_t)0x54 + Xpos);
	}
